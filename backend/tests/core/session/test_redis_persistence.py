...
class TestRedisSessionPersistence:
    ...
    @pytest.mark.asyncio
    async def test_save_serializes_cookies(self, mock_redis):
        """save 应正确序列化 cookies"""
        impl = RedisSessionPersistence.__new__(RedisSessionPersistence)
        impl._redis = mock_redis
        ...
        await impl.save("user123", "jwc", mock_session, 1800)
    ...
    @pytest.mark.asyncio
    async def test_load_returns_session(self, mock_redis):
        """load 应返回重建的 session"""
    ...
    @pytest.mark.asyncio
    async def test_invalidate_deletes_key(self, mock_redis):
        """invalidate 应删除指定 key"""
        impl = RedisSessionPersistence.__new__(RedisSessionPersistence)
        impl._redis = mock_redis

        await impl.invalidate("user123", "jwc")

        mock_redis.delete.assert_called_once_with("session:user123:jwc")

    @pytest.mark.asyncio
    async def test_invalidate_all_deletes_user_keys(self, mock_redis):
        ...
        async def mock_scan_iter(pattern):
            for key in [b"session:user123:jwc", b"session:user123:library"]:
                yield key

        mock_redis.scan_iter = MagicMock(return_value=mock_scan_iter("session:user123:*"))

        await impl.invalidate("user123")
