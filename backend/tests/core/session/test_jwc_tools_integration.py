...
class TestFullLoginFlow:
    ...
    @pytest.mark.asyncio
    async def test_castgc_flow_with_provider(self, persistence, rate_limiter, mock_redis):
        """
        ...
        await manager._save_castgc("user1", "mock_castgc_value")

        # 验证 CASTGC 已保存
        castgc = await manager._get_castgc("user1")
        assert castgc == "mock_castgc_value"
        ...
        with patch.object(JWCSessionProvider, 'fetch_session') as mock_fetch:
            ...
            session = await manager.get_session("user1", "jwc")
    ...
    @pytest.mark.asyncio
    async def test_login_then_different_subsystems(self, persistence, rate_limiter, mock_redis):
        """
        测试登录后访问不同子系统
        ...
        # Step 1: 登录（保存 CASTGC）
        await manager._save_castgc("user1", "shared_castgc")
        ...
        with patch.object(SubsystemSessionProvider, 'get_provider') as mock_get:
            ...
            jwc_session = await manager.get_session("user1", "jwc")
