...
class TestJwcE2E:
    ...
    def test_e2e_get_jwc_session(self, session_manager):
        """测试获取教务系统 Session"""
        print(f"\n尝试使用账号 {settings.cas_username} 登录教务系统...")

        # 首次调用会执行 CAS 登录
        session = session_manager.get_jwc_session(settings.cas_username)
    ...
    def test_e2e_cache_session(self, session_manager):
        """测试 Session 缓存"""
        print(f"\n测试 Session 缓存...")

        # 第一次获取
        session1 = session_manager.get_jwc_session(settings.cas_username)

        # 第二次获取应该从缓存
        session2 = session_manager.get_jwc_session(settings.cas_username)
    ...
    def test_e2e_query_grades(self, session_manager):
        """测试查询成绩（真实网络请求）"""
        print(f"\n尝试查询成绩...")
        ...
        try:
            # 尝试查询成绩（不指定学期，查全部）
            grades = service.get_grades(settings.cas_username, "")
            ...
            if "Cookie 已失效" in str(e):
                ...
                # 清除缓存重试
                session_manager.invalidate_session(settings.cas_username)
...
class TestJwcSessionPersistenceE2E:
    ...
    def test_e2e_persistence(self, tmp_path):
        """测试 Session 持久化"""
        print(f"\n测试 Session 持久化...")
        ...
        session1 = manager1.get_jwc_session(settings.cas_username)
        print(f"✓ 第一次登录成功")
        ...
        # 不需要再次登录
        session2 = manager2.get_jwc_session(settings.cas_username)
        print(f"✓ 第二次从文件加载成功")
