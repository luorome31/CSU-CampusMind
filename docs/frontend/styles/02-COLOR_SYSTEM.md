...
│  │  rgba(83,125,150,0.08) (accent-light)          │    │
│  └─────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────┘
```
...
| **Card** | `#FCFAF5` | 卡片表面 | 高 |
| **Glass** | `rgba(250,248,242,0.92)` | 玻璃态效果 | 中 |
| **Sidebar** | `#F4F2EA` | 侧边栏背景 | 高 |
...
| **Accent Light** | `rgba(83,125,150,0.08)` | 浅色强调背景 | 中 |
...
| 语义 | 色值 | 使用场景 | 对比度 |
|------|------|----------|--------|
| **Success** | `#7BAE7F` | 成功提示、操作完成 | ✓ AA |
| **Coral/Warning** | `#EC8F8D` | 醒目/警告/红色系提示 | ✓ AA |
| **Info** | `#537D96` | 信息提示（使用 accent） | ✓ AA |
...
| 状态 | Success | Coral/Warning |
|------|---------|---------------|
| **Default** | `#7BAE7F` | `#EC8F8D` |
| **Hover** | `#6A9D6E` | `#D87D7A` |
| **Active** | `#5A8C5E` | `#C86B68` |
| **Disabled** | `#7BAE7F` @ 50% | `#EC8F8D` @ 50% |
| **Background** | `rgba(123,174,127,0.12)` | `rgba(236,143,141,0.12)` |
...
/* 低透明度 - 边框 */
.subtle-border {
  border-color: rgba(83, 125, 150, 0.22);
}
...
/* ✗ 错误：仅用颜色区分 */
.status-success { background: #7BAE7F; }
.status-error { background: #EC8F8D; }
/* 色盲用户无法区分 */
```
...
| 变量 | 色值 | 用途 |
|------|------|------|
| `--user-bg` | `rgba(83, 125, 150, 0.08)` | 用户消息气泡背景 |

### 7.2 工具调用卡片

| 变量 | 色值 | 用途 |
|------|------|------|
| `--tool-bg` | `rgba(83, 125, 150, 0.06)` | AI思考/工具调用气泡背景 |
| `--tool-text` | `#6B6F73` | 工具调用卡片文字 |

### 7.3 思考过程区块

| 变量 | 色值 | 用途 |
|------|------|------|
| `--thinking-border` | `rgba(83, 125, 150, 0.22)` | 思考区块左侧指示线 |
